<nav class="lg:hidden relative z-50">
    <div class="flex py-2 justify-between items-center px-4">
        <div class="flex flex-row content-center items-center w-8/12">
            <a href="{{route('welcome')}}">
            <img src="{{asset('images/logo_500.png')}}" alt="logo" class="w-8/12" />
            </a>
        </div>
        <div class="visible flex items-center">
            <button id="open" onclick="MenuHandler(true)" class="focus:outline-none focus:ring-2 focus:ring-black">
                <img src="https://tuk-cdn.s3.amazonaws.com/can-uploader/large_typography_with_gradient_and_glass_effect_Svg2.svg"
                     alt="menu" />
            </button>
            <ul id="list" class="hidden p-2 border-r bg-white absolute rounded top-0 left-0 right-0 shadow mt-24">
                <li
                    class="flex cursor-pointer text-violet-800 text-base leading-3 tracking-normal
                    mt-2 py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none">
                    <a href="{{route('welcome')}}" class="ml-2 focus:outline-none focus:ring-2 focus:ring-black">
                        <span class="font-bold">L'Application</span>
                    </a>
                </li>
                <li class="flex cursor-pointer text-violet-800  text-base leading-3 tracking-normal
                py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none"
                    onclick="dropdownHandler(this)">
                    <a href="{{route('team')}}" class="ml-2 focus:outline-none focus:ring-2 focus:ring-black">
                        <span class="font-bold">La Team</span>
                    </a>
                </li>
{{--                <li--}}
{{--                    class="flex cursor-pointer text-violet-800 text-base leading-3 tracking-normal--}}
{{--                    py-2 hover:text-indigo-700 flex items-center focus:text-indigo-700 focus:outline-none">--}}
{{--                    <a href="javascript: void(0)" class="ml-2 focus:outline-none focus:ring-2 focus:ring-black">--}}
{{--                        <span class="font-bold">Le Tarot d'Iris Claire</span>--}}
{{--                    </a>--}}
{{--                </li>--}}
                <li class="flex cursor-pointer text-violet-800  text-base leading-3 tracking-normal
                py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none"
                    onclick="dropdownHandler(this)">
                    <a href="{{route('blog')}}" class="ml-2 focus:outline-none focus:ring-2 focus:ring-black">
                        <span class="font-bold">Le Blog</span>
                    </a>
                </li>
            </ul>
            <div class="xl:hidden">
                <button id="close" class="hidden close-m-menu focus:ring-2 focus:ring-black focus:outline-none"
                        onclick="MenuHandler(false)">
                    <img src="https://tuk-cdn.s3.amazonaws.com/can-uploader/large_typography_with_gradient_and_glass_effect_Svg3.svg"
                         alt="close" />
                </button>
            </div>
        </div>
    </div>
</nav>
<nav class="f-f-l relative fixed top-0 z-40">
    <div class="relative z-10 mx-auto container hidden w-full px-4 xl:px-0 lg:flex justify-between items-center">
        <div class="flex flex-row content-center items-center w-4/12">
            <a href="{{route('welcome')}}">
            <img src="{{asset('images/logo_500.png')}}" alt="logo" class="w-8/12" />
            </a>
        </div>
        <div class="flex items-center text-purple-800 text-base font-medium">
            <ul class="flex items-center pr-3 xl:pr-12">
                <li class="cursor-pointer text-lg hover:text-orange-300 ease-in">
                    <a href="{{route('welcome')}}" class="focus:outline-none focus:ring-2
                        focus:ring-white">L'Applications</a>
                </li>
                <li class="pl-3 lg:pl-5 xl:pl-8 cursor-pointer hover:text-orange-300  text-lg
                ease-in">
                    <a href="{{ route('team') }}" class="focus:outline-none focus:ring-2
                        focus:ring-white">La Team</a>
                </li>
{{--                <li class="pl-3 lg:pl-5 xl:pl-8 cursor-pointer hover:text-orange-300  text-lg--}}
{{--                ease-in">--}}
{{--                    <a href="javascript:void(0)" class="focus:outline-none focus:ring-2--}}
{{--                        focus:ring-white">Le Tarot Iris Claire</a>--}}
{{--                </li>--}}
                <li class="pl-3 lg:pl-5 xl:pl-8 cursor-pointer hover:text-orange-300  text-lg
                ease-in">
                    <a href="{{route('blog')}}" class="focus:outline-none focus:ring-2
                        focus:ring-white">Le Blog</a>
                </li>
            </ul>

        </div>
        <div class="flex items-center gap-x-4 ">
            <button aria-label="instagram"
                    class="focus:outline-none focus:ring-2 focus:ring-offset-2
                    focus:ring-gray-800  w-8 h-8 flex-shrink-0 bg-violet-800
                    cursor-pointer hover:bg-orange-500 rounded-full flex items-center
                    justify-center">

                <a href="https://www.instagram.com/iris_claire_app/" target="_blank">
                    <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M9.00081 0.233398C6.68327 0.233398 6.39243 0.243215 5.48219 0.283343C4.57374 0.323644 3.95364 0.462973 3.41106 0.667403C2.84981 0.87855 2.37372 1.161 1.8994 1.62066C1.42473 2.08016 1.13317 2.54137 0.914502 3.08491C0.702944 3.61071 0.558942 4.2116 0.518053 5.09132C0.477342 5.97311 0.466675 6.25504 0.466675 8.50015C0.466675 10.7453 0.476986 11.0262 0.518231 11.9079C0.560009 12.788 0.703833 13.3887 0.914679 13.9144C1.13282 14.4581 1.42437 14.9193 1.89887 15.3788C2.37301 15.8386 2.8491 16.1218 3.40999 16.3329C3.95293 16.5373 4.57321 16.6767 5.48148 16.717C6.39171 16.7571 6.68238 16.7669 8.99974 16.7669C11.3175 16.7669 11.6074 16.7571 12.5176 16.717C13.4261 16.6767 14.0469 16.5373 14.5898 16.3329C15.1509 16.1218 15.6263 15.8386 16.1004 15.3788C16.5751 14.9193 16.8667 14.4581 17.0853 13.9145C17.2951 13.3887 17.4391 12.7878 17.4818 11.9081C17.5227 11.0263 17.5333 10.7453 17.5333 8.50015C17.5333 6.25504 17.5227 5.97328 17.4818 5.09149C17.4391 4.21143 17.2951 3.61071 17.0853 3.08508C16.8667 2.54137 16.5751 2.08016 16.1004 1.62066C15.6258 1.16082 15.1511 0.878377 14.5893 0.667403C14.0453 0.462973 13.4249 0.323644 12.5164 0.283343C11.6062 0.243215 11.3164 0.233398 8.99814 0.233398H9.00081ZM8.23525 1.72311C8.46245 1.72277 8.71597 1.72311 9.00077 1.72311C11.2792 1.72311 11.5492 1.73104 12.449 1.77065C13.281 1.8075 13.7326 1.94218 14.0334 2.05533C14.4316 2.20517 14.7155 2.38428 15.014 2.67362C15.3127 2.96295 15.4976 3.23851 15.6526 3.62429C15.7694 3.91535 15.9086 4.3528 15.9464 5.15881C15.9873 6.03026 15.9962 6.29204 15.9962 8.49823C15.9962 10.7044 15.9873 10.9662 15.9464 11.8377C15.9084 12.6437 15.7694 13.0811 15.6526 13.3722C15.4979 13.758 15.3127 14.0327 15.014 14.3218C14.7153 14.6112 14.4318 14.7903 14.0334 14.9401C13.7329 15.0538 13.281 15.1881 12.449 15.225C11.5494 15.2646 11.2792 15.2732 9.00077 15.2732C6.72217 15.2732 6.45212 15.2646 5.55256 15.225C4.72055 15.1878 4.26899 15.0531 3.96801 14.9399C3.56978 14.7901 3.28533 14.611 2.98666 14.3216C2.68799 14.0323 2.5031 13.7574 2.34808 13.3715C2.23128 13.0804 2.09208 12.643 2.05421 11.837C2.01332 10.9655 2.00514 10.7037 2.00514 8.49617C2.00514 6.2886 2.01332 6.0282 2.05421 5.15674C2.09226 4.35073 2.23128 3.91329 2.34808 3.62188C2.50275 3.2361 2.68799 2.96054 2.98666 2.67121C3.28533 2.38187 3.56978 2.20276 3.96801 2.05258C4.26881 1.93891 4.72055 1.80457 5.55256 1.76755C6.33977 1.7331 6.64484 1.72277 8.23525 1.72105V1.72311ZM13.5558 3.09574C12.9905 3.09574 12.5318 3.53956 12.5318 4.08741C12.5318 4.63508 12.9905 5.07942 13.5558 5.07942C14.1212 5.07942 14.5799 4.63508 14.5799 4.08741C14.5799 3.53974 14.1212 3.09574 13.5558 3.09574ZM9.00082 4.25481C6.58071 4.25481 4.61855 6.15564 4.61855 8.50013C4.61855 10.8446 6.58071 12.7446 9.00082 12.7446C11.4209 12.7446 13.3824 10.8446 13.3824 8.50013C13.3824 6.15564 11.4209 4.25481 9.00082 4.25481ZM9.00079 5.74454C10.5717 5.74454 11.8453 6.97818 11.8453 8.50013C11.8453 10.0219 10.5717 11.2557 9.00079 11.2557C7.42975 11.2557 6.15632 10.0219 6.15632 8.50013C6.15632 6.97818 7.42975 5.74454 9.00079 5.74454Z"
                            fill="white"
                        />
                    </svg>
                </a>
            </button>
            <button aria-label="facebook"
                    class="focus:outline-none focus:ring-2 focus:ring-offset-2
                    focus:ring-gray-800  w-8 h-8 flex-shrink-0 bg-violet-800 cursor-pointer
                    hover:bg-orange-500 rounded-full flex items-center justify-center">
                <a href="https://www.facebook.com/people/Iris-Claire/61550798863584/" target="_blank">
                    <svg width="18" height="17" viewBox="0 0 512 512" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            fill="white"
                            d="M134.941,272.691h56.123v231.051c0,4.562,3.696,8.258,8.258,8.258h95.159
	c4.562,0,8.258-3.696,8.258-8.258V273.78h64.519c4.195,0,7.725-3.148,8.204-7.315l9.799-85.061c0.269-2.34-0.472-4.684-2.038-6.44
	c-1.567-1.757-3.81-2.763-6.164-2.763h-74.316V118.88c0-16.073,8.654-24.224,25.726-24.224c2.433,0,48.59,0,48.59,0
	c4.562,0,8.258-3.698,8.258-8.258V8.319c0-4.562-3.696-8.258-8.258-8.258h-66.965C309.622,0.038,308.573,0,307.027,0
	c-11.619,0-52.006,2.281-83.909,31.63c-35.348,32.524-30.434,71.465-29.26,78.217v62.352h-58.918c-4.562,0-8.258,3.696-8.258,8.258
	v83.975C126.683,268.993,130.379,272.691,134.941,272.691z"
                        />
                    </svg>
                </a>
            </button>
            <button aria-label="pinterest"
                    class="focus:outline-none focus:ring-2 focus:ring-offset-2
                    focus:ring-gray-800  w-8 h-8 flex-shrink-0 bg-violet-800 cursor-pointer
                    hover:bg-orange-500 rounded-full flex items-center justify-center">
                <a href="https://www.pinterest.fr/digitalfysas/" target="_blank">
                    <svg width="20" height="17" viewBox="0 0 511.998 511.998" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd"
                              clip-rule="evenodd"
                              fill="white"
                              d="M405.017,52.467C369.774,18.634,321.001,0,267.684,
                                  0C186.24,
                                  0,136.148,33.385,108.468,61.39
	c-34.114,34.513-53.675,80.34-53.675,125.732c0,56.993,23.839,100.737,63.76,117.011c2.68,1.098,5.377,1.651,8.021,1.651
	c8.422,0,15.095-5.511,17.407-14.35c1.348-5.071,4.47-17.582,5.828-23.013c2.906-10.725,0.558-15.884-5.78-23.353
	c-11.546-13.662-16.923-29.817-16.923-50.842c0-62.451,46.502-128.823,132.689-128.823c68.386,0,110.866,38.868,110.866,101.434
	c0,39.482-8.504,76.046-23.951,102.961c-10.734,18.702-29.609,40.995-58.585,40.995c-12.53,0-23.786-5.147-30.888-14.121
	c-6.709-8.483-8.921-19.441-6.222-30.862c3.048-12.904,7.205-26.364,11.228-39.376c7.337-23.766,14.273-46.213,14.273-64.122
	c0-30.632-18.832-51.215-46.857-51.215c-35.616,0-63.519,36.174-63.519,82.354c0,22.648,6.019,39.588,8.744,46.092
	c-4.487,19.01-31.153,132.03-36.211,153.342c-2.925,12.441-20.543,110.705,8.618,118.54c32.764,8.803,62.051-86.899,65.032-97.713
	c2.416-8.795,10.869-42.052,16.049-62.495c15.817,15.235,41.284,25.535,66.064,25.535c46.715,0,88.727-21.022,118.298-59.189
	c28.679-37.02,44.474-88.618,44.474-145.282C457.206,127.983,438.182,84.311,405.017,52.467z"/>
                    </svg>
                </a>
            </button>
            <button aria-label="tiktok"
                    class="focus:outline-none focus:ring-2 focus:ring-offset-2
                    focus:ring-gray-800 w-8 h-8 flex-shrink-0 bg-violet-800 cursor-pointer
                    hover:bg-orange-500 rounded-full flex items-center justify-center">
                <a href="https://www.tiktok.com/@iris.claire8" target="_blank">
                    <svg width="24" height="20" viewBox="0 0 24 24" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            fill="white"
                            d="M21,7H20a4,4,0,0,1-4-4H12V14.5a2.5,2.5,0,1,1-4-2V8.18a6.5,6.5,0,1,0,8,6.32V9.92A8,8,0,0,0,20,11h1Z"
                        />
                    </svg>
                </a>
            </button>
        </div>
    </div>
</nav>

